...
{   //open the file and read it
    ...
    char **pinitial_3 = calloc(12, sizeof(char*));
    ...
    int blocks = -4; //começar a -4 para remover os ! das frases de acolhimento e do especial
    ...
    /*diogo se estas a ler isto é so para saberes que eu estava a pensar que o melhor metodo seria colocar o pespecial no presponses e depois caso
    nao haja match aquilo devolve o count_keyword que vai corresponder a ultima palavra chave que seria o nofound
    e logo ai era so ativar a funcao round-robin para dar a resposta a variar do pespecial mas faz como achares melhor
    que ainda nao sei muito bem como faria isto*/
    while(fgets(buffer, sizeof(buffer), pfile) != NULL){
            buffer[strcspn(buffer, "\r\n")] = '\0';
